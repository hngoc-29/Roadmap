...
export async function getNodeBySlug(roadmapSlug: string, nodeSlug: string) {
  ...
  return { roadmapTitle: roadmap.title, roadmapSlug: roadmap.slug, node: serializeDoc(node) };
}

export async function createRoadmap(data: { title:string; description:string; authorName:string; category?:string }) {
  await connectDB();
  const slug = createSlug(data.title);
  const existing = await Roadmap.findOne({ slug });
  const finalSlug = existing ? `${slug}-${nanoid(4)}` : slug;
  const roadmap = await Roadmap.create({
    title: data.title, slug: finalSlug, description: data.description,
    author: { name: data.authorName }, category: data.category,
    isPublished: false,
    nodes: [{
      id: nanoid(), type: "roadmapNode", position: { x: 250, y: 100 },
      data: { label:"Bước khởi đầu", slug:"buoc-khoi-dau",
              content:"# Bước khởi đầu\n\nThêm nội dung bài học ở đây...",
              description:"Điểm bắt đầu của lộ trình học tập", status:"available", icon:"🚀" },
    }],
    edges: [],
  });
...
export async function saveRoadmapGraph(roadmapId: string, data: { nodes: IRoadmap["nodes"]; edges: IRoadmap["edges"] }) {
  await connectDB();
...
// ──────────────────────────────────────────────
// UPDATE: Cập nhật nội dung 1 Node đã tồn tại trong DB
// ⚠️ KHÔNG gọi hàm này cho node mới tạo cục bộ chưa được saveRoadmapGraph
// ──────────────────────────────────────────────
export async function updateNodeContent(
  ...
  data: {
    ...
    postSlug?: string | undefined;   // ✅ MỚI: link tới /blog/[slug]
  }
) {
  await connectDB();
  ...
  if (data.postSlug !== undefined) updateFields["nodes.$.data.postSlug"] = data.postSlug ?? "";  // ✅ MỚI
...
export async function getAllRoadmapSlugs() {
  ...
  return serializeDoc(roadmaps) as unknown as Array<{
    slug: string; nodes: Array<{ data: { slug: string } }>; updatedAt: string;
  }>;
}
