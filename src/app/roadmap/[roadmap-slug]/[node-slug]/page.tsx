...
// ============================================================
// ✅ FIX: force-dynamic + bỏ isPublished filter trong getNodeBySlug
//         → Trang bài học của draft roadmap không còn 404
...
export default async function NodeLessonPage({
  ...
  // ✅ FIX: getNodeBySlug đã bỏ isPublished filter → node của draft roadmap load được
  const result = await getNodeBySlug(roadmapSlug, nodeSlug);
  if (!result) notFound();
      ...
      <article className="max-w-4xl mx-auto px-4 py-10">
        <header className="mb-8">
          ...
          <div className="flex flex-wrap items-center gap-3 text-sm">
            <span className="flex items-center gap-1 text-muted-foreground">📖 {readingTime}</span>
            {nodeData.estimatedTime && (
              <span className="flex items-center gap-1 text-muted-foreground">⏱️ {nodeData.estimatedTime}</span>
            )}
            {nodeData.difficulty && (
        ...
        {/* MDX Content - Server render, SEO-ready */}
