...
export default function RoadmapBuilder({
  ...
  const onNodeClick: NodeMouseHandler = useCallback(
    (event, node) => {
      ...
      if (mode === "view") {
        ...
        // ✅ Ưu tiên contentSlug → /content/[slug]
        // Fallback → /roadmap/[roadmap-slug]/[node-slug]
  ...
  const handleAddNode = useCallback(() => {
    ...
    // Tự động mở modal edit cho node vừa tạo
    setTimeout(() => {
  ...
  // ── Callback nhận edges mới từ modal (tab Kết nối) ──
  ...
  // Nodes cho tab Kết nối (không cần position, chỉ cần id + data)
            ...
            {mode === "edit" && (
              <>
                <button
                  ...
                  title="Thêm node mới (tự động mở editor)"
                >
                  ➕ Thêm node
                </button>
                <button
